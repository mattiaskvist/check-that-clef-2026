...
def main():
    ...
    st.write("Index the collection, optionally merge custom documents, and retrieve top-5 article matches for your tweet.")
    ...
    if st.session_state.get("is_indexed", False) and not st.session_state.get("is_polling", False):
        
        actual_count = st.session_state.get("doc_count", 0)
        summary_len = len(st.session_state.get("docs_summary",))
        
        st.success(f"Successfully indexed {actual_count} documents on GPU!")
        
        if actual_count > 50:
            st.warning(f"Data payload truncated from {actual_count} to 50 rows to prevent WebSocket saturation.")
