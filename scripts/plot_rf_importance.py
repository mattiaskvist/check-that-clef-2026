...
def main():
    ...
    try:
        ...
        print("Please ensure you are authenticated (run: uv run hf auth login) or have set HF_TOKEN.")
        return
    ...
    if not hasattr(model, "feature_importances_"):
        print("The loaded model does not have 'feature_importances_'. Is it a Random Forest?")
        return

    importances = model.feature_importances_
    
    if len(importances) != len(FEATURE_NAMES):
        print(f"Warning: Model expects {len(importances)} features, but we defined {len(FEATURE_NAMES)}.")
        # If lengths don't match, just use generic names
        names = FEATURE_NAMES if len(importances) == len(FEATURE_NAMES) else [f"Feature {i}" for i in range(len(importances))]
    else:
        names = FEATURE_NAMES
    ...
    bars = plt.barh(sorted_names, sorted_importances, color="skyblue", edgecolor="black")
