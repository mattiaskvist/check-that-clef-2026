...
- Clean up the codebase and make it more modular and reusable. The current implementation is somewhat monolithic, especially in the `check_that_modal.py` script, which contains a lot of logic for the entire pipeline. It would be beneficial to refactor the code into separate modules for retrieval, fusion, and re-ranking, which would make it easier to maintain and extend in the future. This would also allow us to easily swap out different components (e.g., trying different fusion techniques or re-ranking models) without having to modify a large monolithic script. @mattiaskvist is working on this task.
