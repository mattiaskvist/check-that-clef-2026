...
class RRFFuser(BaseFuser):
    ...
    def _rrf_with_scores(
        ranked_lists: list[list[int]], k: int = 60, top_k: int | None = None,
        ...
        top_ids = [doc_id for doc_id, _ in sorted_docs]
...
class RandomForestFuser(BaseFuser):
    ...
    def _cache_path(self, cache_dir: str) -> str:
        ...
        return os.path.join(cache_dir, "rf_fusion_models", f"rf_{self._fingerprint}.pkl")
    ...
    def load(
        ...
        self._fingerprint = self._compute_fingerprint(
            dense_model_name, sparse_config, train_split,
            self.rf_params, self.global_model, self.candidate_top_k,
        )
        path = self._cache_path(cache_dir)
    ...
    def train(
        ...
        for lang in languages:
            ...
            for i in tqdm(range(len(tweets)), desc=f"[RF Train] Building features {lang.upper()}"):
                ...
                union_candidates = list(dict.fromkeys(
                ...
                features = FeatureGenerator.build_query_features(
                    union_candidates, dense_scores, dense_ranks,
                    sparse_scores, sparse_ranks, rrf_scores, rrf_ranked,
                    include_rrf=True,
                )
        ...
        if self.global_model:
            ...
            for lang in languages:
                X_lang, y_lang = lang_features[lang]
                print(f"[RF Train] Training {lang.upper()} model on {len(X_lang)} samples...")
                clf = RandomForestClassifier(**self.rf_params)
                clf.fit(np.array(X_lang, dtype=np.float32), np.array(y_lang, dtype=np.float32))
