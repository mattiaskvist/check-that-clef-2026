...
from full_pipeline.fusions import (
    BaseFuser,
    FeatureGenerator,
    RandomForestFuser,
    RRFFuser,
...
class TestFeatureGenerator:
    def test_builds_correct_features_with_rrf(self):
        ...
        features = FeatureGenerator.build_query_features(
            candidates, dense_scores, dense_ranked,
            sparse_scores, sparse_ranked,
            rrf_scores, rrf_ranked, include_rrf=True,
        )
    ...
    def test_builds_correct_features_without_rrf(self):
        ...
        features = FeatureGenerator.build_query_features(
            candidates, dense_scores, dense_ranked,
            sparse_scores, sparse_ranked,
            rrf_scores, rrf_ranked, include_rrf=False,
        )
...
def _make_mock_dense_retriever(n_docs: int = 20, n_queries: int = 10):
    ...
    for cache_name_suffix in ["en", "de", "fr"]:
        for prefix in ["queries_train_", "queries_"]:
            ...
            ranks_matrix = [
                list(np.argsort(s)[::-1]) for s in scores_matrix
            ]
...
class TestRandomForestFuser:
    ...
    def test_train_and_fuse_per_language(self):
        ...
        for lang in ["en", "de", "fr"]:
            dense_ranks, dense_scores = dense.search_with_scores(0, cache_name=f"queries_{lang}")
            sparse_ranks, sparse_scores = sparse.search_with_scores(0, cache_name=f"sparse_queries_{lang}")
    ...
    def test_train_and_fuse_global(self):
        ...
        sparse_ranks, sparse_scores = sparse.search_with_scores(0, cache_name="sparse_queries_en")
    ...
    def test_fingerprint_changes_with_config(self):
        fp1 = RandomForestFuser._compute_fingerprint(
            "model/a", {"k1": 2.5, "b": 0.85, "stemmer": "lancaster"},
            "train", {"n_estimators": 100}, False, 500,
        )
        fp2 = RandomForestFuser._compute_fingerprint(
            "model/b", {"k1": 2.5, "b": 0.85, "stemmer": "lancaster"},
            "train", {"n_estimators": 100}, False, 500,
        )
        fp3 = RandomForestFuser._compute_fingerprint(
            "model/a", {"k1": 1.5, "b": 0.75, "stemmer": "porter"},
            "train", {"n_estimators": 100}, False, 500,
        )
        fp4 = RandomForestFuser._compute_fingerprint(
            "model/a", {"k1": 2.5, "b": 0.85, "stemmer": "lancaster"},
            "train", {"n_estimators": 200}, False, 500,
        )
